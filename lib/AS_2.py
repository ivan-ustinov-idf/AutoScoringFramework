...
def gini_month_selection(X: pd.DataFrame, df: pd.DataFrame, gini_min: float=0.05,
                         num_bad_months: int=2, target_name: str='target') -> Tuple[list, pd.DataFrame]:
    '''
    ...
    '''
    df_x_month = pd.concat([X.reset_index(drop=True), df[['date_requested', target_name]].reset_index(drop=True)], axis=1)
    df_x_month['requested_month_year'] = df_x_month.date_requested.map(lambda x: str(x)[:7])
    vars_woe = X.columns
